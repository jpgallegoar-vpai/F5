...
def infer_process(
    ...
    max_chars1 = int(speech_rate * batch_size)  # 10 seconds per batch
    max_chars2 = int(0.06911 / batch_size)
...
def calculate_duration_samples(text_length, target_sample_rate, hop_length, speech_rate=1.0):
    ...
    """
    base_char_weight = 0.06911  # Base weight for normal-length texts
    reference_length = 60  # Reference text length (~12s worth of text)
    exp_factor = 1  # Controls scaling curve steepness
    
    # Calculate exponential scaling based on text length
    if text_length <= reference_length:
        scale = exp_factor * np.exp(-text_length / reference_length) + 1
    else:
        scale = 1.0
    
    # Apply speech rate adjustment
    char_weight = (base_char_weight * scale) / speech_rate
    
    duration_samples = int(text_length * char_weight * target_sample_rate / hop_length)
    
    # Ensure minimum duration of 1 second
    min_duration_samples = int(1.25 * target_sample_rate / hop_length)
